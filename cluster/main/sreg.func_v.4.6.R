...
#-------------------------------------------------------------------
filter.ols <- function(Y,S,D,G.id,Ng,X,s,d)
  #-------------------------------------------------------------------
{
...
#-------------------------------------------------------------------
lm.iter <- function(Y,S,D,G.id,Ng,X,exp.option =FALSE)
  #-------------------------------------------------------------------
{ 
  ...
  for (d in 0:max(D))
  {
    for (s in 1:max(S))
    {
      data.filtered <- filter.ols(Y,S,D,G.id,Ng,X,s,d)
...
#-------------------------------------------------------------------
tau.hat <- function(Y,S,D,G.id,Ng,X,model, exp.option = FALSE)
  #-------------------------------------------------------------------
{
...
as.var <- function(model, fit)
{
  ...
  for (d in 1:length(fit$tau.hat))
  {
    ...
    #print(Y.g.mean.1)
    #print(Y.g.mean.0)
    #print(mean(Y.g.mean.1) - mean(Y.g.mean.0))
    Xi.hat.1 <- Xi.tilde.1 - Xi.1.mean
    Xi.hat.0 <- Xi.tilde.0 - Xi.0.mean
    Xi.hat.2 <- Y.g.bar.cl.1 - Y.g.bar.cl.0 #- tau.est[d] * (Ng.Sg.mean - mean(data.filter$Ng))
    #Xi.hat.2.alt <- (Y.g.bar.cl.1 - Y.g.bar.cl.0) - tau.est[d] * (Ng.Sg.mean)
    #print(mean(Xi.hat.2^2))
    #print(mean(Xi.hat.2.alt^2))
...
#-------------------------------------------------------------------
sreg <- function(Y,S,D,G.id,Ng,X, exp.option = FALSE)
  #-------------------------------------------------------------------
{
  n <- length(Y)
  if (exp.option == FALSE)
  {
    model <- lm.iter(Y,S,D,G.id,Ng,X, exp.option = FALSE)
    fit <- tau.hat(Y,S,D,G.id,Ng,X,model, exp.option = FALSE)
  }else{
    model <- lm.iter(Y,S,D,G.id,Ng,X, exp.option = TRUE)
    fit <- tau.hat(Y,S,D,G.id,Ng,X,model, exp.option = TRUE)
  }
  tau.est <- fit$tau.hat
  #as.var.est <- as.var(Y,S,D,X,model,tau.est)
  se.rob <- as.var(model,fit)
  ...
  #-------------------------------------------------------------------
{
  ...
  cat("Saturated Model Estimation Results under CAR with linear adjustments and clusters \n")                  
