...
#%##%##%##%##
#%# v.3.2 #%#
#%# Generalizing for the multiple treatment case
#%##%##%##%##
#-------------------------------------------------------------------
#%##%##%##%##%##%##%##%##%##%##%##%##%##%##%##%##%##%##%##%##%##%##%
#%##%##%##%##%##%#      I. ATEs estimation    #%##%##%##%##%##%##%##
...
#-------------------------------------------------------------------
indicator <- function(variable, type)
  #-------------------------------------------------------------------
{
  if (min(variable) == 0)
  {
...
#-------------------------------------------------------------------
filter.ols <- function(Y,S,D,X,s,d)
#-------------------------------------------------------------------
{
  X <- as.matrix(X)
...
{ 
  ...
  for (d in 0:max(D))
  {
    for (s in 1:max(S))
    {
      data.filtered <- filter.ols(Y,S,D,X,s,d)
...
#-------------------------------------------------------------------
pi.hat <- function(S,D,X)
#-------------------------------------------------------------------
{
  n <- length(D)
  data <- data.frame(S,D,X)
...
#-------------------------------------------------------------------
tau.hat <- function(Y,S,D,X, model)
#-------------------------------------------------------------------
{
...
# Variance Estimator
#-------------------------------------------------------------------
 as.var <- function(Y,S,D,X, model, tau)
{
  ...
  for (d in 1:max(D))
  {
    ...
    Xi.tilde.1 <- (1 - (1/data.bin$pi)) * mu.hat.d - mu.hat.0 + 
      (data.bin$Y / data.bin$pi) #- tau[d] * (1 - (1/data.bin$pi) +  (data.bin$A / data.bin$pi)) #- tau[d] * (data.bin$A / data.bin$pi)
    
    Xi.tilde.0 <- ((1 / (1 - data.bin$pi)) - 1) * mu.hat.0 + mu.hat.d -
      (data.bin$Y / (1 - data.bin$pi)) #- tau[d] * (1 - (data.bin$A / (1 - data.bin$pi))) #- tau[d] * (-data.bin$A / (1 - data.bin$pi))
...
#-------------------------------------------------------------------
sreg <- function(Y,S,D,X)
  #-------------------------------------------------------------------
{
  n <- length(Y)
  model <- lm.iter(Y,S,D,X)
  tau.est <- tau.hat(Y,S,D,X,model)
  #as.var.est <- as.var(Y,S,D,X,model,tau.est)
  se.rob <- as.var(Y,S,D,X,model,tau.est)
