...
#-------------------------------------------------------------------
sreg <- function(Y,D,S,G.id,Ng,X, exp.option = FALSE)
  #-------------------------------------------------------------------
{
  n <- length(Y)
  if (exp.option == FALSE)
  ...
  res.list <- list('tau.hat' = tau.est,
                   ...
                   'CI.left' = CI.left,
                   'CI.right'= CI.right,
                   'data' = data.frame(Y,D,S,G.id,Ng,X))
  return(res.list)
}
...
#------------------------------------------------------------------
gen.data.pot <- function(Ng, G, tau.vec, sigma1=sqrt(1), 
  ...
  #------------------------------------------------------------------
{
  ...
  m.0 <- gamma.vec[1] * Z.g.2 + gamma.vec[2] * x_1 + gamma.vec[3] * x_2 #+ 0.7 * Ng.norm #+ gamma.vec[4] * x_3 + gamma.vec[5] * x_4 # G=500 obs
